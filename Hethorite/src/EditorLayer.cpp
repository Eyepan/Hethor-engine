...
#include "Hethor/Scene/SceneSerializer.h"
#include "Hethor/Utils/PlatformUtils.h"
#include "Hethor/Math/Math.h"
#include "Hethor/Scripting/ScriptEngine.h"
#include "Hethor/Renderer/Font.h"
...
namespace Hethor
{
  ...
  void EditorLayer::OnAttach()
  {
    ...
    else
    {
      ...
      // If no project is opened, close Hethorite
  ...
  {
    std::string filepath = FileDialogs::OpenFile("Hethor Project (*.hproj)\0*.hproj\0");
    if (filepath.empty())
      return false;
  ...
  {
    std::string filepath = FileDialogs::OpenFile("Hethor Scene (*.hethor)\0*.hethor\0");
    if (!filepath.empty())
      OpenScene(filepath);
  }
  ...
  void EditorLayer::OpenScene(const std::filesystem::path &path)
  {
    ...
    if (path.extension().string() != ".hethor")
    {
  ...
  {
    std::string filepath = FileDialogs::SaveFile("Hethor Scene (*.hethor)\0*.hethor\0");
    if (!filepath.empty())
    {
